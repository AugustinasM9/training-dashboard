...
- Proxies intervals.icu API (JSON + binary FIT files)
...
def build_icu_url(path):
    """Build intervals.icu URL. Activity paths bypass /athlete/{id}/"""
    if path.startswith('/activity/') or path.startswith('/activities/'):
        return f"https://intervals.icu/api/v1{path}"
    return f"https://intervals.icu/api/v1/athlete/{ICU_ID}{path}"

def proxy_icu(path, query, method='GET', body=None):
    """Proxy JSON request to intervals.icu"""
    url = build_icu_url(path)
    if query:
        url += '?' + query
...
def proxy_icu_binary(path, query):
    """Proxy binary file (FIT, TCX, GPX, etc) from intervals.icu"""
    url = build_icu_url(path)
    if query:
        url += '?' + query
    print(f"[ICU-BIN] GET {url}")
    ...
    try:
        ...
        except: body_str = '(binary error)'
        print(f"[ICU-BIN ERROR {e.code}] {body_str[:200]}")
        return e.code, body, 'application/json'
    except Exception as e:
        print(f"[ICU-BIN EXCEPTION] {e}")
        return 500, json.dumps({'error': str(e)}).encode(), 'application/json'

def proxy_claude(body_data):
    if not ANTHROPIC_KEY:
        return 500, json.dumps({'error': 'ANTHROPIC_API_KEY not configured on server.'})
...
class Handler(BaseHTTPRequestHandler):
    ...
    def send_binary(self, status, data, ctype):
        self.send_response(status)
    ...
    def do_GET(self):
        ...
        if path.startswith('/fit/'):
            if not check_auth(self.headers):
                ...
                return
            activity_id = path[5:]
            status, body, ctype = proxy_icu_binary(f'/activity/{activity_id}/fit-file', parsed.query)
            if status == 200:
                self.send_binary(status, body, ctype)
            else:
                self.send_json(status, body.decode('utf-8', errors='replace') if isinstance(body, bytes) else body)
            return
